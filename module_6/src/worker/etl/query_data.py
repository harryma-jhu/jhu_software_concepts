...
def get_avg_metrics(cur):
    query = sql.SQL("""
        SELECT 
            COALESCE(ROUND(AVG(gpa)::numeric, 2), 0), 
            COALESCE(ROUND(AVG(gre)::numeric, 2), 0), 
            COALESCE(ROUND(AVG(gre_v)::numeric, 2), 0), 
            COALESCE(ROUND(AVG(gre_aw)::numeric, 2), 0) 
        FROM applicants;
    """)
    cur.execute(query)
    result = cur.fetchone()
    return result if result else (0, 0, 0, 0) # Guarantee a 4-item tuple
...
def get_jhu_cs_masters(cur):
    ...
    """)
    cur.execute(query, ("%Johns Hopkins%", "%Masters%", "%Computer Science%"))
    return cur.fetchone()[0]
...
def get_elite_phd(cur):
    ...
    cur.execute(query, ("%2026%", "Accepted", "%PhD%", "%Computer Science%", list(schools)))
    return cur.fetchone()[0]
